...
function Kibitzer (options) {
    ...
    this._legislator.log.consumer().pump(this._islander)

    // While we read messages off of the Islander, uh, but it is not necessary!?!
    this.consumer = this._islander.log.consumer()

    // Used to record events generated during playback.
    this._recording = { paxos: [], islander: [] }

    this._connectors = { legislator: new Queue, islander: new Queue }

    this._legislator.outbox.consumer().pump(this._connectors.legislator)
    this._islander.outbox.consumer().pump(this._connectors.islander)

    this._outboxes = {
        legislator: this._connectors.legislator.consumer(),
        islander: this._connectors.islander.consumer()
    }

    this.messenger = new Outbox
    ...
    async([function () {
        ...
        var loop = async(function () {
            async(function () {
                ...
                async(function () {
                    var properties = this._legislator.properties[this._legislator.government.majority[0]]
                    this.messenger.send({
                        to: properties,
                        body: {
                ...
                async(function () {
                    pulse.route.forEach(function (id) {
                        if (id == this._legislator.id) {
                            ...
                            async(function () {
                                var properties = this._legislator.properties[id]
                                this.messenger.send({
                                    to: properties,
                                    body: { type: 'receive', pulse: pulse }
                                }, async())
                            }, function (response) {
...
Kibitzer.prototype._naturalize = cadence(function (async, post) {
    ...
    if (!outcome.enqueued && outcome.leader != null && post.hops == 0) {
        var properties = this._legislator.properties[outcome.leader]
        post.hops++
        this.messenger.send({ to: properties, body: post }, async())
    } else {
        return [ outcome ]
    }
})
